...
internal class Connection : NSObject, NSURLConnectionDataDelegate, NSURLConnectionDelegate {
  ...
  init?(request: Request) {
    assert(!(!request.isAsynchronous && NSThread.isMainThread()), "\n\nWe turned off the ability to send synchronous requests on the main thread, as it may cause a lot of non-obvious, subtle bugs. \nPlease send synchronous requests from other threads. \nThank you for your understanding and good luck in the use of our library.\n\n")
